...
export class EmployeeDAOPostgres implements IDAO<Employee> {
    async create(employee: Employee): Promise<ObjectResponse<Employee>> {
        const query = 'INSERT INTO public.employee(\n' +
            'fk_physical_location, name, last_name, email, telephone, role, password, doc_type, identification)\n' +
    ...
    async delete(employeeToDelete: Employee): Promise<boolean> {
        const query = 'UPDATE employee SET active = false WHERE pk_id = $1'
        try {
            let pool = await PostgresConnection.getInstance().getPool()
            let res = await pool.query({
                text: query,
                values: [employeeToDelete.id]
            })
